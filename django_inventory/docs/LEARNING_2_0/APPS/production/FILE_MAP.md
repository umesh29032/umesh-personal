...
- `pool_service.py` (S4/Phase 2) — thin dispatcher over `handler.pool_good` / `handler.materialize_pool` (no stage-name conditionals). `pool_good(sr)` → `{(color_id,size_id): good}` (cutting→APSCPB, downstream→`StagePoolSnapshot`); `materialize_stage_pool(sr)` write-once SPS freeze (cutting=no-op); `clear_stage_pool(sr)` for reopen. POOL-ONLY. Draw-down (Phase 3) will add the D2 advisory lock.
