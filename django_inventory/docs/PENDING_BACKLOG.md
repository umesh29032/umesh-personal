...
# PENDING BACKLOG — engineering/product open items (2026-06-14)

> **Scope boundary (stated 2026-07-13, Phase-7 Q-A6 / finding F-B-06):** this file =
> ENGINEERING + PRODUCT open items (soak-gated S6/R11-class, deferred features, polish).
> **Campaign findings/INFO rows live in [DEPLOYMENT_BACKLOG.md](DEPLOYMENT_BACKLOG.md)**;
> live campaign state = [DEPLOYMENT_CAMPAIGN_STATUS.md](DEPLOYMENT_CAMPAIGN_STATUS.md).
> Neither list supersedes the other — two registers, two scopes.
> Big-picture order lives in
> [IMPLEMENTATION_ROADMAP_PDD_V1.md](IMPLEMENTATION_ROADMAP_PDD_V1.md) (the frozen R1–R11
> register; **current state = [MANUFACTURING_V1_FREEZE.md](MANUFACTURING_V1_FREEZE.md)**;
> POST_C1 review = historical detail source). This file
> gathers EVERY pending item — large and small — with its source. Update when
> an item ships; never delete, strike.
> **🔒 OPERATIONAL FOUNDATION FROZEN 2026-07-05** (R1→A360 + polish + closeout;
> permanent rules in PDD amendments + RBAC.md) — new work builds ON it; no
> refactors of frozen modules without explicit owner approval.
...
- **🟢 PRODUCTION-TRUTH FOUNDATION STREAM CLOSED 2026-06-14** — S1 · S1.1 · S3 · S4 · F1–F4 · Browser E2E · S5 all DONE + committed (641 tests green; prod 0037–0042 + expense 0010/0011). **S6 (`reported_quantity` retirement + column DROP) intentionally DEFERRED** to post-deploy + soak + MT-1 backup + rollback review. Next architecture stream = Layering → Pattern Design → Cutting → **Stitching** flow design (design-only). **Documentation debt** from the /find-canonical audit (≈18 archive + ~13 update + 5 manifest topics) is LOGGED, NOT EXECUTED → [DOCUMENTATION_DEBT_2026_06_14](DOCUMENTATION_DEBT_2026_06_14.md); run as one docs-sync sprint when scheduled (manifest is CI-guarded; archive moves last).
- **FOUNDATION S5 (M-6 finalize BLOCK + allocation-bound rollout safety) DONE 2026-06-14** ([S5_DESIGN_RECEIPT](S5_DESIGN_RECEIPT_2026_06_14.md); runbook [ENFORCEMENT_ROLLOUT_RUNBOOK](ENFORCEMENT_ROLLOUT_RUNBOOK_2026_06_14.md)) — **M-6 BLOCK:** `finalize` refuses when a stage settled MORE than produced (over_allocated beyond tolerance) once **`ENFORCE_SETTLEMENT_RECONCILIATION`** (default False=WARN) is on, unless a **super-admin** supplies an audited **override reason** (stamped on `SettlementReconciliationEvidence` — migration **expense 0011**: +`override_reason` +`overridden_by`). `SETTLEMENT_RECONCILIATION_TOLERANCE` (abs pieces, default 0). over_allocated only (H1); quantity-only (settled good vs `cost_quantity_snapshot`, no rate/earning); pre-check rolls back the atomic finalize on block (nothing books); **actionable diagnostics** (stage + settled vs produced + over-by + corrective paths). Thin UI: super-admin override-reason field on the settlement finalize form. **Rollout safety (S4-005):** `pool_service.preview_bound_violations` (over_bound + unallocated completions) + `preview_allocation_bound` management command (exit 1 on violations — verified flagging real dev-data) + non-blocking `bound_soft_warning` on the worker report screen + the runbook (pre-flip checklists for BOTH flags). **Both flags default OFF + additive migration → reversible (deploy OFF → soak → resolve → enable).** Invariants intact (4 concerns independent; WSA production-only; settlement the only money boundary; **golden ₹225 byte-identical** — clean Adda finalizes either flag state). Tests: 6 BLOCK + 5 preview/soft-warn. **Soak-gated enablement; S6 (reported_quantity retirement) next.**
- **FOUNDATION S4 Phase 5 (Era-A reopen guard + pool-clear) DONE 2026-06-14 → S4 COMPLETE** ([S4_PHASE5_RECEIPT](S4_PHASE5_RECEIPT_2026_06_14.md)) — uniform downstream-consumer guard in the reopen skeleton (`_shared._downstream_consumer_guard`, resolves M-4-3): refuse reopen of stage S while any stage `order > S` has a **non-voided `WorkerStageAllocation`** OR a **completed/verified `WorkerStageContribution`** OR (future) `AlterCase`. Transitive; names the **furthest** blocking stage + blocker type + corrective action (**actionable error**: reverse settlement / void allocations / reopen downstream). `clear_stage_pool(sr)` wired into the skeleton (reopened pool-producing stage clears its SPS). Reverse-first; **strict** (no orphaned allocations / no silent upstream pool edit) yet **recoverable** (every blocker → explicit audited reverse/void/reopen, peeled furthest-inward). No model/migration. 7 reopen-guard tests (all 6 scenarios + actionable message). Golden ₹225 byte-identical; full suite green. **Inert in current flow** (cutting's only downstream = barcode NONE/auto). **→ S4 (production-truth pool/allocation foundation) is COMPLETE: D1 grain + 2 StagePoolSnapshot + 3 WorkerStageAllocation/draw-down + 4 complete-bound + 5 reopen-guard. Built ahead of its first consumer (stitching); `ENFORCE_ALLOCATION_BOUND=False` until staging-validated.** Next foundation step: S5 (M-6 finalize BLOCK) / S6 (reported_quantity retirement) per MASTER_PLAN_V2 — both soak-gated.
