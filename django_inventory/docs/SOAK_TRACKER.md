...
| 2026-06-11 | SEC | **Assignment-list filter inconsistency**: layering assign list = skill-filtered (worker3 absent); cutting workspace assign list = UNFILTERED (manager1, super_admin, no-skill worker3 all offered). Also note: worker-report view gates on assignment (task) only — a no-skill worker WITH a task could report (assignment-implies-authorized; defensible, but Skill∧Assignment is the stated stage rule). | **OWNER TRIAGE (F7): EDGE/ACCESS-CONTROL · fix-after-P3.** Real issue = DUPLICATED assignment policy (cutting role-only; 3 forms hardcode skill names; `Stage.access_by_skill` is the canonical source). **Locked future direction: assignable pool = Stage.access_by_skill ∪ management, active users only, ALL stage forms deriving from that one source** (R1 open-closed; kills the duplication). Current behavior NOT documented as intended. No action now. |
