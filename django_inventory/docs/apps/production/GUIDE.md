...
| File | Role |
|---|---|
| `worker_task_service.py` | ★ chokepoint: WST/WSC sole writer, expected freeze, verified qty (P1), auto-cancel resolve. **R10-B: report lines accept optional alter/missing observations (S3 columns; good stays the ONLY payable — D2)**. ⚠️ `set_stage_workers` contract: caller MUST be `@transaction.atomic` — **P19A C-1 (2026-07-20): `stages/layering/service.py`'s `start_layering` silently lost its decorator to a helper inserted between `@transaction.atomic` and the `def` (v1.0.0 streams-redesign commit `90c1f2f3`) → `TransactionManagementError` 500 + partial committed writes on EVERY layering roster update; fixed by re-binding the decorator; class is invisible to `TestCase` (tests run inside a transaction) so it's pinned by `test_p19a_regressions.LayeringStartTransactionTest` (TransactionTestCase + real POST)** |
| `adda_service.py` | Adda create (race-safe per-product counter; **BUG-B1 fix 2026-07-20: code-gen now SKIPS any already-existing `{code}-{NNN}` before insert — out-of-band codes (migrations/imports/dev scripts) desync `adda_counter`, and without the skip the INSERT raised an unhandled `IntegrityError` (500) AND the rolled-back txn wedged creation permanently; loop runs under the select_for_update product lock**; **F-2 polish 2026-07-05: NO auto-assign — empty roster, manager assigns via panel; spec-D6 no-skilled-users guard retained; C-1 closeout: the retro-tag other-half `sync_layering_workers_for_skill` is REMOVED from layering/service — manager assignment = the ONLY roster source, PDD amendment 4**) + stage advance. **Adda-delete safety (owner rule 2026-07-22): `cancel_adda` (SOFT — super_admin, status→CANCELLED, open-task auto-cancel via F3 resolver, `ADDA_CANCELLED` history, money-guarded by settlement) + `delete_adda` (HARD — super_admin, pristine-only: refuses on completed-stage/settlement/SWA/allocation/reported-good/barcode via `Adda.deletion_block_reason()`; leaf-first teardown then `adda.delete()` in one txn — a leftover cross-app PROTECT rolls back to a clean "cancel instead"). Model `delete()` guard = admin/shell backstop. ADR-0012; pins `test_adda_delete` 19/19**) |
...
| `pool_service.py` | ★ S4/P2-P5 + **AE-1 (bundle model 2026-07-20)**: THE piece-pool chokepoint — sole writer of StagePoolSnapshot + WorkerStageAllocation. `pool_good`/`materialize_stage_pool`/`clear_stage_pool` (snapshot) + `allocate`(explicit PARTIAL, `mode='partial'`)/**`allocate_whole`(DEFAULT — `qty=available`, `mode='whole'`, grabs remainder)**/`void_allocation`/`available` (draw-down, D2 advisory lock 5375) + `check_allocation_bound` (**AE-1: HARD + ALWAYS-ON, flag retired; Σ(good+alter+missing+damaged)≤allocated per reported dim; unallocated pair refused; PRODUCER stages (no upstream) skipped**) + `preview_bound_violations` (now audits pre-AE-1 historical rows). POOL-ONLY, decoupled from cost/rate/settlement |
| `bundle_service.py` | **AE-1/AE-3 read-model (no table, no writes):** projects the Manufacturing Bundle (adda,colour,size) over pool+WSA — `bundles_for_stage` (AE-2 manager board; dropdown filters `available>0` in-template) + `worker_bundles` (per-SR worker cards) + **`my_assigned_work(user)` (AE-3 cross-Adda cards + workload summary)** + `stage_snapshot` (AE-4 super-admin). `_bundle_status` = the single (locked,remaining,completed)→status map shared by worker_bundles+stage_snapshot. Generic, no stage names (Platform genericity pass 2026-07-20: dead `unassigned_bundles`/`whole_holders`/`partial_holders` dropped) |
| `worker_task_service.report_bundle` | **AE-3 per-bundle submit:** upserts ONE bundle's contribution (keeps the worker's other bundles), immediate per-bundle over-report bound, completes the task ONLY when every allocated bundle is reported. Full multi-line save+complete path unchanged |
| `bundle_service.stage_snapshot` + `AddaSnapshotView` | **AE-4 Super-Admin Production Snapshot** (`/production/addas/<code>/snapshot/`, management-only, linked from Adda detail): per pool stage = 7 stage totals + per-bundle rollup (total/assigned/unassigned/completed/progress/holders) + per-worker×bundle detail (mode/allocated/completed/remaining/expected₹/status/started/updated). Read-model; numbers reconcile to DB truth (tested) |
...
| `access_service.py` | skill-gating reads + **`eligible_stage_workers(stage_code)` = THE shared picker population (F-4: active ∩ Stage.access_by_skill — picker == gate; all 4 stage start-forms use it + `generic_stage/views_ctx.py`'s `eligible_workers` ctx key). Roster-picker audit 2026-07-24: query verified correct (single `.filter()` call, no leak); coverage gap found+closed — `F4SharedPickerTests` only pinned the 4 bespoke forms, not the `generic_stage` context path, now covered by `GenericPanelRosterPickerTests` in `test_r10b_generic_stage.py`** |
...
`worker_report_views.py` (★ phone report + AddaReportReviewView P1; **C-3 closeout 2026-07-05: report requires active assignment AND live Stage Access — hub revocation closes the path instantly**; **P19A C-2 fix 2026-07-20: `_resolve` is now LANE-AWARE — pre-production stages carry one SR per lane, the old single-row `.get()` 500'd (`MultipleObjectsReturned`, pre-permission, all roles) on every multi-lane Adda; resolution = worker's own non-cancelled tasks pick the SR (open task preferred), optional `?stream=<id>` narrows, no-SR → 404, no-task → 403; pinned by `test_p19a_regressions` 6/6**),
