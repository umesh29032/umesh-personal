...
| `adda_service.py` | Adda create (race-safe per-product counter; **BUG-B1 fix 2026-07-20: code-gen now SKIPS any already-existing `{code}-{NNN}` before insert — out-of-band codes (migrations/imports/dev scripts) desync `adda_counter`, and without the skip the INSERT raised an unhandled `IntegrityError` (500) AND the rolled-back txn wedged creation permanently; loop runs under the select_for_update product lock**; **F-2 polish 2026-07-05: NO auto-assign — empty roster, manager assigns via panel; spec-D6 no-skilled-users guard retained; C-1 closeout: the retro-tag other-half `sync_layering_workers_for_skill` is REMOVED from layering/service — manager assignment = the ONLY roster source, PDD amendment 4**) + stage advance |
