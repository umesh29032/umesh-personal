---
id: url-card-account-confirm-email
type: url-card
status: generated
owner: generated
scope: route — url:account_confirm_email
anchors: config/config/urls.py
verified: graph:5d159ece1df0
---

# URL card — `account_confirm_email`
...
| Namespace | — (none) |
| Mount | `/accounts/^confirm-email/(?P<key>[-:\w]+)/$` |
| Pattern | `^confirm-email/(?P<key>[-:\w]+)/$` |
| Named | true |
...
| Defined in | `/accounts/^confirm-email/(?P<key>[-:\w]+)/$` |
| Vendor | true |
