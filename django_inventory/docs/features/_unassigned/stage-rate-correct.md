---
id: url-card-production-stage-rate-correct
type: url-card
status: generated
owner: generated
scope: route — url:production:stage-rate-correct
anchors: config/config/urls.py
verified: graph:56207d76ed26
---

# URL card — `stage-rate-correct`
...
| Field | Value |
|---|---|
| URL name | `stage-rate-correct` |
| Namespace | `production` |
| Mount | `/production/addas/<str:code>/stage-rates/<int:sr_id>/<int:role_id>/correct/` |
| Pattern | `addas/<str:code>/stage-rates/<int:sr_id>/<int:role_id>/correct/` |
| Named | true |
...
| Field | Value |
|---|---|
| View | `StageRateCorrectView` |
| Module | `production.views.rate_views` |
| Defined in | `config/production/views/rate_views.py` |
| Vendor | false |

## App

Derived from the view module (`production.views.rate_views` → app `production`; a render rule, not a graph edge).
