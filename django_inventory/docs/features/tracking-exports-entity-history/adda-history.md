---
id: url-card-tracking-adda-history
type: url-card
status: generated
owner: generated
scope: route — url:tracking:adda-history
anchors: config/config/urls.py
verified: graph:4ed4a2180bcf
---

# URL card — `adda-history`
...
| Field | Value |
|---|---|
| URL name | `adda-history` |
| Namespace | `tracking` |
| Mount | `/tracking/history/adda/<str:adda_code>/` |
| Pattern | `history/adda/<str:adda_code>/` |
| Named | true |
...
| Field | Value |
|---|---|
| View | `AddaHistoryView` |
| Module | `inventory.views.tracking_history` |
| Defined in | `config/inventory/views/tracking_history.py` |
| Vendor | false |

## App

Derived from the view module (`inventory.views.tracking_history` → app `inventory`; a render rule, not a graph edge).
